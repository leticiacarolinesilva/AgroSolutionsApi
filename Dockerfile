...
COPY ["src/AgroSolutions.UnitTests/AgroSolutions.UnitTests.csproj", "src/AgroSolutions.UnitTests/"]

# Restore packages
RUN dotnet restore "src/AgroSolutions.Api/AgroSolutions.Api.csproj"
RUN dotnet restore "src/AgroSolutions.UnitTests/AgroSolutions.UnitTests.csproj"
...
RUN dotnet build "src/AgroSolutions.UnitTests/AgroSolutions.UnitTests.csproj" -c Release

# Test stage
FROM build AS test
WORKDIR /src
RUN dotnet test "src/AgroSolutions.UnitTests/AgroSolutions.UnitTests.csproj" -c Release --no-build
